import { createContext, FunctionComponent, ReactNode, useState, useEffect, useContext } from "react";
...
export const PlacesContext: FunctionComponent<{children: ReactNode}> = ({ children }) => {
    const [places, setPlaces] = useState<BasicPlace[]>([]);
    useEffect(() => {
        cachedApi.getPlaces().then(i => setPlaces(i.Reply.Result));
    }, []);

    console.log("placesContext", places);
